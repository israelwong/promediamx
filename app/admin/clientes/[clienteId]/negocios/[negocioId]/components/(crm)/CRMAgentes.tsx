...
import {
    ...
    editarAgenteCRM,
    eliminarAgenteCRM
} from '@/app/admin/_lib/crmAgente.actions'; // Asegúrate que las acciones estén aquí
import { Agente } from '@/app/admin/_lib/types'; // Importar tipo
...
interface AgenteConDatos extends Agente {
    _count?: { Lead?: number }; // Ejemplo si añades conteo
}
...
export default function CRMAgentes({ crmId }: Props) {
    ...
    const fetchAgentes = useCallback(async (isInitialLoad = false) => {
        ...
        setError(null);
        try {
            const data = await obtenerAgentesCRM(crmId);
            setAgentes(data || []);
        } catch (err) {
    ...
    const openModal = (mode: 'create' | 'edit', agente?: AgenteConDatos) => {
        ...
        setModalFormData(mode === 'edit' && agente ?
            ...
            } :
            {
                ...
                email: '',
                telefono: '',
                password: '', // Campo para creación
                rol: 'agente_ventas', // Rol por defecto
                status: 'activo'
            }
        );
        setIsModalOpen(true);
        ...
        try {
            ...
            if (modalMode === 'create') {
                ...
                result = await crearAgenteCRM({
                    crmId: crmId, // Asociar al CRM actual
    ...
    const handleModalDelete = async () => {
        ...
        // Aquí podrías añadir lógica para no eliminar el último agente o un agente específico si es necesario
        ...
        <div className={containerClasses}>
            ...
            <div className={headerClasses}>
                <h3 className="text-base font-semibold text-white whitespace-nowrap flex items-center gap-2">
                    <UserCog size={16} /> Agentes del CRM
                </h3>
                <button onClick={() => openModal('create')} className={buttonPrimaryClasses} title="Crear nuevo agente">
                ...
                {loading ? (
                    ...
                    <ul className='space-y-2'>
                        {agentes.map((agente) => {
                            ...
                            // const leadCount = agente._count?.Lead ?? 0; // Usar conteo si se incluye
                            return (
                                <li key={agente.id} className={listItemClasses}>
                                    <div className="flex-grow mr-2 overflow-hidden">
                                        ...
                                        {/* <p className="text-xs text-zinc-500 truncate">{agente.telefono || 'Sin teléfono'}</p> */}
                                        <div className='flex items-center gap-2 mt-1'>
                                            ...
                                            <span className="text-xs text-zinc-500 bg-zinc-700 px-1.5 py-0.5 rounded">
                                                {agente.rol || 'Agente'} {/* Mostrar Rol */}
                                            </span>
                                            {/* Opcional: Mostrar conteo de leads */}
                            ...
                            <div className={modalBodyClasses}>
                                ...
                                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                                    ...
                                    <div>
                                        ...
                                        <input type="email" id="modal-email" name="email" value={modalFormData.email || ''} onChange={handleModalFormChange} className={inputBaseClasses} required disabled={isSubmittingModal || modalMode === 'edit'} /> {/* Deshabilitar email en edición */}
                                ...
                                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
                                    <div>
                                        ...
                                        <select id="modal-rol" name="rol" value={modalFormData.rol || 'agente_ventas'} onChange={handleModalFormChange} className={`${inputBaseClasses} appearance-none`} disabled={isSubmittingModal}>
                                            {/* Poblar con roles definidos o dejarlo simple */}
