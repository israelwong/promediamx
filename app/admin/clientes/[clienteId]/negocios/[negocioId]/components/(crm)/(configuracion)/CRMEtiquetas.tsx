...
import {
    ...
    editarEtiquetaCRM,
    eliminarEtiquetaCRM,
    ordenarEtiquetasCRM // Importar acción de ordenar
} from '@/app/admin/_lib/crmEtiqueta.actions';
import { EtiquetaCRM } from '@/app/admin/_lib/types'; // Importar tipo
...
function SortableTagItem({ etiqueta, onEditClick }: { etiqueta: EtiquetaCRMConOrden, onEditClick: (et: EtiquetaCRMConOrden) => void }) {
    ...
    const colorDotClasses = "w-3 h-3 rounded-full border border-zinc-500 flex-shrink-0"; // Círculo de color
    ...
    return (
        <li ref={setNodeRef} style={style} className={listItemClasses}>
            ...
            {/* <span className={`text-xs px-1.5 py-0.5 rounded ${etiqueta.status === 'activo' ? 'bg-green-500/20 text-green-400' : 'bg-zinc-600 text-zinc-400'}`}>{etiqueta.status}</span> */}
...
export default function CRMEtiquetas({ crmId }: Props) {
    ...
    const colorInputClasses = `${inputBaseClasses} h-10 p-1 cursor-pointer`; // Estilo para input de color
    ...
    const fetchEtiquetas = useCallback(async (isInitialLoad = false) => {
        ...
        setError(null);
        try {
            const data = await obtenerEtiquetasCRM(crmId);
