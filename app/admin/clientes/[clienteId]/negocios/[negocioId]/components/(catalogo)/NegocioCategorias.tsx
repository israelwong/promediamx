...
import NegocioCategoriaTip from './NegocioCategoriaTip';
...
function SortableCategoryItem({ categoria, onEditClick }: { categoria: NegocioCategoriaConOrden, onEditClick: (cat: NegocioCategoriaConOrden) => void }) {
    ...
    return (
        <li ref={setNodeRef} style={style} className={listItemClasses}>
            <button {...attributes} {...listeners} data-dndkit-drag-handle className="cursor-grab touch-none text-zinc-500 hover:text-zinc-300 flex-shrink-0 focus:outline-none focus:ring-1 focus:ring-blue-500 rounded" aria-label="Mover categoría" onClick={(e) => e.stopPropagation()}>
                <GripVertical size={18} />
            </button>
            <div className="flex-grow mr-2 overflow-hidden" onClick={() => onEditClick(categoria)} title={`Editar: ${categoria.nombre}`}> {/* Hacer clickeable el contenido */}
                <p className="text-sm font-medium text-zinc-200 truncate">{categoria.nombre}</p>
...
export default function NegocioCategorias({ negocioId }: Props) {
    ...
    const listContainerClasses = "flex-grow overflow-y-auto space-y-0"; // Quitado space-y para que el botón quede pegado
    // const buttonPrimaryClasses = "..."; // No se usa en cabecera
    ...
    const fetchCategorias = useCallback(async (isInitialLoad = false) => {
        ...
        try {
            ...
            const categoriasConOrden = (data || []).map((cat, index) => ({ ...cat, orden: cat.orden ?? index + 1, }));
            setCategorias(categoriasConOrden);
        } catch (err) { console.error("Error al obtener las categorias:", err); setError("No se pudieron cargar las categorías."); setCategorias([]); }
    ...
    const openModal = (mode: 'create' | 'edit', categoria?: NegocioCategoriaConOrden) => {
        setModalMode(mode); setCategoriaParaEditar(mode === 'edit' ? categoria || null : null);
    ...
    const handleModalFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (modalMode === 'create') { const nuevoOrden = categorias.length + 1; await crearNegocioCategoria({ ...dataToSend, negocioId: negocioId, orden: nuevoOrden }); }
            else if (modalMode === 'edit' && categoriaParaEditar?.id) { await actualizarNegocioCategoria(categoriaParaEditar.id, dataToSend); }
            await fetchCategorias(); closeModal();
        } catch (err) { console.error(`Error al ${modalMode === 'create' ? 'crear' : 'actualizar'} categoria:`, err); const message = err instanceof Error ? err.message : "Ocurrió un error"; setModalError(`Error: ${message}`); setIsSubmittingModal(false); }
    };
    const handleModalDelete = async () => {
        ...
        if (confirm(`¿Estás seguro de eliminar la categoría "${categoriaParaEditar.nombre}"? Esto podría afectar ítems asociados.`)) {
            setIsSubmittingModal(true); setModalError(null);
            try { await eliminarNegocioCategoria(categoriaParaEditar.id); await fetchCategorias(); closeModal(); }
            catch (err) { console.error("Error eliminando categoria:", err); const message = err instanceof Error ? err.message : "Ocurrió un error"; setModalError(`Error al eliminar: ${message}`); setIsSubmittingModal(false); }
        }
    };

    // --- Manejador Drag End para Categorías (sin cambios) ---
    const handleDragEndCategorias = useCallback(async (event: DragEndEvent) => {
        const { active, over } = event;
        if (over && active.id !== over.id) {
            const oldIndex = categorias.findIndex((cat) => cat.id === active.id); const newIndex = categorias.findIndex((cat) => cat.id === over.id);
            ...
            try { await actualizarOrdenNegocioCategorias(ordenData); }
            catch (saveError) { console.error('Error al guardar el orden de categorías:', saveError); setError('Error al guardar el nuevo orden.'); setCategorias(categorias); } // Revertir
    ...
    const renderInternalContent = () => {
        ...
        // Mostrar lista o mensaje si está vacía
        return (
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEndCategorias}>
                <SortableContext items={categorias.map(cat => cat.id)} strategy={verticalListSortingStrategy}>
                    {categorias.length === 0 && !error ? (
                        <div className="flex flex-col items-center justify-center text-center py-6"><ListChecks className="h-6 w-6 text-zinc-500 mb-2" /><p className='text-zinc-400 italic text-sm'>No hay categorías definidas.</p></div>
                    ) : (
                        <ul className='divide-y divide-zinc-700'> {/* Usar divide para líneas */}
    ...
    return (
        <div className={containerClasses}>
            ...
            {/* --- NUEVO: Botón Crear Categoría Apilado --- */}
            {/* Mostrar solo si no está cargando y no hay error */}
            {!loading && !error && (
                <div className="mt-auto pt-3"> {/* mt-auto empuja al fondo, pt-3 da espacio */}
                    <button
            ...
            <NegocioCategoriaTip /> {/* Bloque Explicativo Mejorado para Categorías */}


            {/* Modal para Crear/Editar Categoría (sin cambios) */}
            {isModalOpen && (
                <div className={modalOverlayClasses} onClick={closeModal}>
                    <div className={modalContentClasses} onClick={(e) => e.stopPropagation()}>
                        <div className={modalHeaderClasses}> <h3 className="text-lg font-semibold text-white"> {modalMode === 'create' ? 'Crear Nueva Categoría' : 'Editar Categoría'} </h3> <button onClick={closeModal} className="text-zinc-400 hover:text-white" aria-label="Cerrar modal"> <XIcon size={20} /> </button> </div>
