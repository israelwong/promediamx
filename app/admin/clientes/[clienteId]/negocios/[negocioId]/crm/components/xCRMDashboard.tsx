...
// Importar iconos
import { Loader2, AlertTriangle, LayoutDashboard, DatabaseZap } from 'lucide-react';
...
export default function CRMDashboard({ negocioId }: Props) {
    ...
    const tabs: { id: TabId; label: string; icon: React.ElementType; component: React.ReactNode }[] = [
        { id: 'estadisticas', label: 'CRM', icon: LayoutDashboard, component: crmData?.id ? <CRMEstadisticas crmId={crmData.id} negocioId={negocioId} /> : null },
        // { id: 'agenda', label: 'LeadForm', icon: Keyboard, component: crmData?.id ? <CRMCamposPersonalizados crmId={crmData.id} /> : null },
