...
import {
    obtenerEtapasPipelineCRM, // <-- Acción refactorizada
    crearPipelineCRM,
    editarPipelineCRM,
    eliminarPipelineCRM,
    ordenarPipelineCRM
} from '@/app/admin/_lib/crmPipeline.actions'; // Ajusta ruta!
import { PipelineCRM } from '@/app/admin/_lib/types'; // Ajusta ruta!
...
export default function CRMPipeline({ negocioId }: Props) {
    ...
    const fetchEtapas = useCallback(async (isInitialLoad = false) => {
        ...
        try {
            ...
            const result = await obtenerEtapasPipelineCRM(negocioId);

            if (result.success && result.data) {
                // Almacenar crmId y etapas
                setCrmId(result.data.crmId);
                setEtapas((result.data.etapas || []).map((et, index) => ({ ...et, orden: et.orden ?? index + 1 })));
                if (!result.data.crmId) {
                    setError("CRM no encontrado para este negocio. Debe configurarse primero.");
                }
            } else {
                throw new Error(result.error || "Error desconocido al cargar etapas.");
            }
        } catch (err) {
            console.error("Error al obtener las etapas:", err);
            setError(`No se pudieron cargar las etapas: ${err instanceof Error ? err.message : "Error desconocido"}`);
            setEtapas([]);
            setCrmId(null);
        } finally {
    ...
    const openModal = (mode: 'create' | 'edit', etapa?: PipelineCRMConOrden) => {
        ...
        if (mode === 'create' && !crmId) {
            setError("No se puede crear una etapa sin un CRM configurado.");
            return;
        }
        setModalMode(mode);
    ...
    const handleModalFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (modalMode === 'create') {
                if (!crmId) throw new Error("crmId no está disponible para crear la etapa."); // Verificación extra
            ...
            if (result?.success) {
                await fetchEtapas(); // Recargar lista
                closeModal();
    ...
    const handleDragEnd = useCallback(async (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            setEtapas(finalEtapas); // Optimista
        ...
        <div className={containerClasses}>
            ...
            <div className={headerClasses}>
                <h3 className="text-base font-semibold text-white whitespace-nowrap flex items-center gap-2">
                    <Workflow size={16} /> Configurar Pipeline de Ventas
                </h3>
                <button
                    onClick={() => openModal('create')}
                    className={buttonPrimaryClasses}
                    title={!crmId ? "Configura el CRM primero" : "Crear nueva etapa"}
            ...
            <div className={listContainerClasses}>
                ...
                ) : etapas.length === 0 && !error && crmId ? ( // Mostrar solo si hay crmId pero no etapas
