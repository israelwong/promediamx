                ...
                {!isLoading && !error && conversations.length > 0 && (
                    <ul className="space-y-1"> {/* Quitado flex-grow y overflow-y-auto de aquí */}
                        {conversations.map((conv) => {
                            ...
                            return (
                                <li key={conv.id}>
                                    <Link href={`${basePath}/${conv.id}`} className={`flex items-center gap-3 p-2.5 rounded-md transition-colors ${isActive ? 'bg-zinc-700 shadow-md' : 'hover:bg-zinc-700/60'}`}>
                                        ...
                                        <div className="flex-grow overflow-hidden">
                                            <div className="flex justify-between items-center">
                                                <div className="flex items-center gap-1.5 truncate">
                                                    <span className="flex-shrink-0" title={`Canal: ${conv.canalOrigen || '?'}`}>{getChannelIcon(conv.canalOrigen)}</span>
                                        ...
                                        <span className={`text-xs flex-shrink-0 ${isActive ? 'text-zinc-300' : 'text-zinc-500'}`}>
