...
// --- Actions and Types ---
import {
    obtenerImagenesGaleriaOferta,
    añadirImagenGaleriaOferta,
    actualizarDetallesImagenGaleriaOferta,
    eliminarImagenGaleriaOferta,
    actualizarOrdenGaleriaOferta
} from '@/app/admin/_lib/ofertaGaleria.actions'; // Asegúrate que la ruta es correcta
import type { OfertaGaleria } from '@/app/admin/_lib/types'; // Importa el tipo de Prisma
import { ImagenOfertaGaleriaDetallesInput, ImagenOfertaOrdenData } from '@/app/admin/_lib/ofertaGaleria.actions'; // Importa tipos específicos
// --- Icons ---
...
interface Props {
    ofertaId: string; // Recibe el ID de la oferta
}

// --- Componente Miniatura Arrastrable (Adaptado para OfertaGaleria) ---
function SortableOfertaImageThumbnail({ id, imagen, index, onEdit, onDelete, onImageClick }: {
    id: string;
    imagen: OfertaGaleria; // Tipo adaptado
    index: number;
    onEdit: (img: OfertaGaleria) => void;
    ...
    return (
        <div ref={setNodeRef} style={style} className={`${thumbnailClasses} ${isDragging ? 'ring-2 ring-blue-500 shadow-xl' : ''}`}>
            ...
            <Image
                src={imagen.imageUrl} // Campo correcto
                alt={imagen.altText || `Imagen Oferta ${index + 1}`} // Texto alt adaptado
                fill
...
// --- Componente Modal para Editar Detalles (Adaptado para OfertaGaleria) ---
function EditOfertaImageDetailsModal({ imagen, isOpen, onClose, onSave }: {
    imagen: OfertaGaleria | null; // Tipo adaptado
    isOpen: boolean;
    onClose: () => void;
    onSave: (id: string, data: ImagenOfertaGaleriaDetallesInput) => Promise<void>; // Tipo adaptado
}) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            onClose();
        } catch (err) { setModalError(err instanceof Error ? err.message : "Error al guardar"); }
        ...
        <div className={modalOverlayClasses} onClick={onClose}>
            <div className={modalContentClasses} onClick={(e) => e.stopPropagation()}>
                <div className={modalHeaderClasses}>
                    <h3 className="text-base font-semibold text-white">Editar Detalles de Imagen</h3>
                ...
                <div className={modalBodyClasses}>
                    ...
                    <div className='flex justify-center mb-2 max-h-48'>
                    ...
                    <div>
                        ...
                        <textarea id="descripcion" value={descripcion} onChange={(e) => setDescripcion(e.target.value)} className={`${inputClasses} min-h-[60px]`} disabled={isSaving} rows={2} maxLength={250} placeholder="Contexto adicional..." />
                    </div>
                </div>
                <div className={modalFooterClasses}>
                    ...
                    <button type="button" onClick={handleSave} className={`${buttonClasses} bg-blue-600 hover:bg-blue-700 focus:ring-blue-500`} disabled={isSaving}>
...
// --- Componente Principal OfertaGaleria ---
export default function OfertaGaleria({ ofertaId }: Props) {
    const [imagenes, setImagenes] = useState<OfertaGaleria[]>([]); // Tipo adaptado
    ...
    const [editingImage, setEditingImage] = useState<OfertaGaleria | null>(null); // Tipo adaptado

    // Clases Tailwind reutilizadas
    const containerClasses = "p-4 bg-zinc-900/30 rounded-lg border border-zinc-700/50 min-h-[300px] h-full flex flex-col";
    ...
    const gridContainerClasses = "grid grid-cols-3 sm:grid-cols-4 gap-2 flex-grow overflow-y-auto pr-1";
    ...
    // --- Clase para el fondo del loading ---
    const loadingContainerClasses = "flex-grow flex items-center justify-center text-zinc-400 bg-zinc-800 rounded-b-lg"; // Fondo zinc-900
    ...
    const fetchImagenes = useCallback(async () => {
        ...
        try {
            const data = await obtenerImagenesGaleriaOferta(ofertaId);
            setImagenes(data || []);
        } catch (err) { setError(err instanceof Error ? err.message : "Error al cargar galería de oferta"); setImagenes([]); }
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            // Llamar a la acción específica de oferta
            const result = await añadirImagenGaleriaOferta(ofertaId, file, file.name); // Pasar alt/desc si se capturan
    ...
    const handleDelete = async (imagenId: string) => {
        ...
        if (confirm("¿Eliminar imagen permanentemente?")) {
            ...
            try {
                // Llamar a la acción específica de oferta
                const result = await eliminarImagenGaleriaOferta(imagenId);
    ...
    const handleEditDetails = (imagen: OfertaGaleria) => { setEditingImage(imagen); };

    const handleSaveDetails = async (id: string, data: ImagenOfertaGaleriaDetallesInput) => {
        try {
            // Llamar a la acción específica de oferta
            const result = await actualizarDetallesImagenGaleriaOferta(id, data);
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            const ordenData: ImagenOfertaOrdenData[] = finalOrder.map(({ id, orden }) => ({ id, orden: orden as number }));

            setIsSavingOrder(true); setError(null);
            try {
                // Llamar a la acción específica de oferta
                const result = await actualizarOrdenGaleriaOferta(ofertaId, ordenData);
    ...
    return (
        <div className={containerClasses}>
            ...
            <div className={headerClasses}>
                <h3 className="flex items-center gap-2"><GalleryHorizontal size={14} /> Galería de Oferta ({imagenes.length}/{MAX_IMAGES})</h3>
            ...
            {/* Contenido Galería */}
            {loading ? (
                // --- ESTADO DE CARGA CON FONDO ZINC 900 ---
                <div className={loadingContainerClasses}>
                    <Loader2 className="h-5 w-5 animate-spin mr-2" /><span>Cargando galería...</span>
                </div>
            ) : (
