...
// Ajusta rutas
import {
    obtenerImagenesGaleriaItem,
    añadirImagenGaleriaItem,
    actualizarDetallesImagenGaleria,
    eliminarImagenGaleriaItem,
    actualizarOrdenGaleriaItem
} from '@/app/admin/_lib/itemGaleria.actions'; // Asegúrate que las rutas sean correctas
import { ItemCatalogoGaleria, ImagenGaleriaDetallesInput, ImagenOrdenData } from '@/app/admin/_lib/types'; // Asegúrate que las rutas sean correctas
import { Loader2, Trash2, PlusCircle, GripVertical, Star, Pencil, Save, XIcon, AlertCircle, GalleryHorizontal } from 'lucide-react'; // Añadido UploadCloud
...
// --- Componente Miniatura Arrastrable (Sin cambios) ---
function SortableImageThumbnail({ id, imagen, index, onEdit, onDelete, onImageClick }: {
    id: string;
    imagen: ItemCatalogoGaleria;
    index: number;
    onEdit: (img: ItemCatalogoGaleria) => void;
    ...
    return (
        <div ref={setNodeRef} style={style} className={`${thumbnailClasses} ${isDragging ? 'ring-2 ring-blue-500 shadow-xl' : ''}`}>
            ...
            <Image
                src={imagen.imageUrl} alt={imagen.altText || `Imagen ${index + 1}`} fill className={imageClasses}
                sizes="(max-width: 640px) 25vw, (max-width: 1024px) 15vw, 10vw" priority={isPortada} loading={isPortada ? 'eager' : 'lazy'}
                onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} // Oculta si hay error de carga
            />
            {/* Overlay para acciones y click de lightbox */}
...
// --- Componente Modal para Editar Detalles (Sin cambios) ---
function EditImageDetailsModal({ imagen, isOpen, onClose, onSave }: {
    imagen: ItemCatalogoGaleria | null;
    isOpen: boolean;
    onClose: () => void;
    onSave: (id: string, data: ImagenGaleriaDetallesInput) => Promise<void>;
}) {
    ...
    useEffect(() => {
        if (imagen) {
            setAltText(imagen.altText || '');
            setDescripcion(imagen.descripcion || '');
        }
        setModalError(null); // Resetear error al abrir/cambiar imagen
    }, [imagen]);
    ...
    const handleSave = async () => {
        ...
        setModalError(null);
        try {
            await onSave(imagen.id, { altText, descripcion });
        ...
        <div className={modalOverlayClasses} onClick={onClose}>
            <div className={modalContentClasses} onClick={(e) => e.stopPropagation()}>
                <div className={modalHeaderClasses}>
                    <h3 className="text-base font-semibold text-white">Editar Detalles Metadatos</h3>
                    <button onClick={onClose} className="p-1 text-zinc-400 hover:text-white rounded-full hover:bg-zinc-700"><XIcon size={18} /></button>
                </div>
                <div className={modalBodyClasses}>
                    ...
                    <div className='flex justify-center mb-2 max-h-48'> {/* Limitar altura preview */}
                        <Image src={imagen.imageUrl} alt="Preview" width={400} height={400} className="object-contain rounded border border-zinc-600" />
                    </div>
                    <div>
                        <label htmlFor="altText" className={labelClasses}>Texto Alternativo (Alt)</label>
                        <input type="text" id="altText" value={altText} onChange={(e) => setAltText(e.target.value)} className={inputClasses} disabled={isSaving} maxLength={150} placeholder="Describe la imagen..." />
                        <p className="text-xs text-zinc-500 mt-1">Importante para accesibilidad.</p>
                    </div>
                    <div>
                        <label htmlFor="descripcion" className={labelClasses}>Descripción (Lightbox)</label>
                        <textarea id="descripcion" value={descripcion} onChange={(e) => setDescripcion(e.target.value)} className={`${inputClasses} min-h-[60px] white-space: pre-wrap;`} disabled={isSaving} rows={2} maxLength={250} placeholder="Contexto adicional..." />
                    </div>
                </div>
                <div className={modalFooterClasses}>
                    <button type="button" onClick={onClose} className={`${buttonClasses} bg-zinc-600 hover:bg-zinc-500 focus:ring-zinc-400`} disabled={isSaving}>Cerrar</button>
                    <button
                        type="button"
                        onClick={handleSave}
...
// --- Componente Principal ItemGaleria ---
export default function ItemGaleria({ itemId }: Props) {
    const [imagenes, setImagenes] = useState<ItemCatalogoGaleria[]>([]);
    ...
    const [editingImage, setEditingImage] = useState<ItemCatalogoGaleria | null>(null);

    // Clases Tailwind
    const containerClasses = "bg-zinc-800 rounded-md p-4 rounded-lg border border-zinc-700/50 min-h-[300px] h-full flex flex-col";
    ...
    const gridContainerClasses = "grid grid-cols-3 sm:grid-cols-4 gap-2 flex-grow overflow-y-auto pr-1"; // Grid para miniaturas
    ...
    // --- NUEVA CLASE para el indicador de carga ---
    ...
    const fetchImagenes = useCallback(async () => {
        ...
        try { const data = await obtenerImagenesGaleriaItem(itemId); setImagenes(data || []); }
        catch (err) { setError(err instanceof Error ? err.message : "Error al cargar galería"); setImagenes([]); }
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        event.target.value = ''; // Resetear input para permitir subir el mismo archivo de nuevo
        ...
        try {
            ...
            if (file.size > maxSize) throw new Error(`Máximo ${Math.round(maxSize / 1024 / 1024)}MB por imagen`);

            // Llamada a la acción de añadir imagen
            const result = await añadirImagenGaleriaItem(itemId, file, file.name);
            if (!result.success || !result.data) throw new Error(result.error || "Error al añadir imagen");

            // Actualizar estado local si la subida fue exitosa
    ...
    const handleDelete = async (imagenId: string) => {
        ...
        if (confirm("¿Eliminar imagen permanentemente?")) {
            ...
            setSuccessMessage(null);
            try {
                const result = await eliminarImagenGaleriaItem(imagenId);
    ...
    const handleEditDetails = (imagen: ItemCatalogoGaleria) => { setEditingImage(imagen); };

    const handleSaveDetails = async (id: string, data: ImagenGaleriaDetallesInput) => {
        try {
            const result = await actualizarDetallesImagenGaleria(id, data);
            ...
            console.error("Error save details:", err);
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            const ordenData: ImagenOrdenData[] = finalOrder.map(({ id, orden }) => ({ id, orden: orden as number }));

            // Guardar orden en backend
            setIsSavingOrder(true);
            setError(null);
            try {
                const result = await actualizarOrdenGaleriaItem(itemId, ordenData);
                ...
                // Revertir UI si falla (opcional, o recargar)
                fetchImagenes(); // Recargar para asegurar consistencia
            } finally {
    ...
    return (
        <div className={containerClasses}>
            {/* Input oculto */}
            <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/png, image/jpeg, image/webp" disabled={isUploading || !canUpload} />

            {/* Cabecera */}
            <div className={headerClasses}>
                <h3 className="flex items-center gap-2"><GalleryHorizontal size={14} /> Galería ({imagenes.length}/{MAX_IMAGES})</h3>
                <div className='flex items-center gap-2'>
                    ...
                    {/* Botón Añadir (deshabilitado durante carga) */}
            ...
            {/* Contenido Galería */}
            {loading ? (
                <div className="flex-grow flex items-center justify-center text-zinc-400"><Loader2 className="h-5 w-5 animate-spin mr-2" /><span>Cargando...</span></div>
            ) : (
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                    <SortableContext items={imagenes.map(img => img.id)} strategy={rectSortingStrategy}>
                        <div className={gridContainerClasses}>
                            {/* --- NUEVO: Indicador de Carga Visual --- */}
                            {isUploading && (
                                <div className={uploadingIndicatorClasses}>
                            ...
                            {/* Botón/Dropzone para añadir (solo si se puede y no está cargando) */}
                            {canUpload && !isUploading && (
                                <div onClick={handleAddClick} className={`${dropzoneClasses} aspect-square flex items-center justify-center flex-col border-zinc-700 hover:border-blue-500 self-start`}>
                            ...
                            {imagenes.map((img, index) => (
                                <SortableImageThumbnail
                                    ...
                                    onDelete={isDeleting === img.id ? () => { } : handleDelete} // Deshabilita delete mientras se borra
                                    onImageClick={openLightbox} // Usa el nuevo handler
                                />
                            ))}
