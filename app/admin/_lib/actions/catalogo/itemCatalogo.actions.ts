...
export async function eliminarItemCatalogo(
    ...
    try {
        ...
        // Buscar los videos asociados al ítem
        const videos = await prisma.itemCatalogoVideos.findMany({
            where: { itemCatalogoId: itemId },
            select: { videoUrl: true }
        });

        // Eliminar los videos del storage (Supabase)
        if (videos.length > 0) {
            const deleteVideoPromises = videos.map(video =>
                video.videoUrl ? eliminarImagenStorage(video.videoUrl) : Promise.resolve({ success: true })
            );
            const videoResults = await Promise.allSettled(deleteVideoPromises);
            videoResults.forEach((result, index) => {
                if (result.status === 'rejected' || (result.status === 'fulfilled' && !result.value.success)) {
                    console.warn(`Fallo al eliminar video ${videos[index].videoUrl} del storage: ${result.status === 'rejected' ? result.reason : (result.value && 'error' in result.value ? result.value.error : 'Error desconocido')}`);
                }
            });
        }

        // Luego elimina los registros de videos en la base de datos
        await prisma.itemCatalogoVideos.deleteMany({
            where: { itemCatalogoId: itemId }
        });
        ...
        return { success: false, error: "No se pudo eliminar el ítem. Es probable que esté asociado a un paquete. Primero eliminalo del paquete y luego intenta eliminarlo nuevamente" };
    }
}
