...
//     let textoNormalizado = fechaHoraStrEntrada.toLowerCase().trim();
//     console.log(`[parsearFechaHoraInteligente] Input: "${fechaHoraStrEntrada}", Normalizado: "${textoNormalizado}", Opciones: ${JSON.stringify(opciones)}`);

//     let fechaBase: Date | null = null;
//     let horaExtraida: { hours: number; minutes: number } | null = null;

//     const partesHora = extraerYParsearHora(textoNormalizado);
//     if (partesHora) {
//         horaExtraida = partesHora;
//         const matchHoraEnString = textoNormalizado.match(/\d{1,2}(\s*[:\.]\s*\d{1,2})?(\s*(am|pm|de\s+la\s+mañana|de\s+la\s+tarde|de\s+la\s+noche))?/i);
//         if (matchHoraEnString) {
//             textoNormalizado = textoNormalizado.replace(matchHoraEnString[0], "").trim().replace(/^a\s+las\s+/, "").trim();
//         }
//     }
//     console.log(`[parsearFechaHoraInteligente] Hora extraída: ${JSON.stringify(horaExtraida)}, Texto restante para fecha: "${textoNormalizado}"`);

//     textoNormalizado = textoNormalizado.replace(/^(para el|el)\s+/, "").trim();

//     if (textoNormalizado.includes("mañana")) fechaBase = addDays(hoy, 1);
//     else if (textoNormalizado.includes("pasado mañana")) fechaBase = addDays(hoy, 2);
//     else if (textoNormalizado.includes("hoy") || (textoNormalizado === "" && horaExtraida)) fechaBase = hoy;
//     else if (textoNormalizado.match(/pr[oó]ximo\s+lunes/i) || textoNormalizado === "lunes") fechaBase = nextMonday(ahora);
//     else if (textoNormalizado.match(/pr[oó]ximo\s+martes/i) || textoNormalizado === "martes") fechaBase = nextTuesday(ahora);
//     else if (textoNormalizado.match(/pr[oó]ximo\s+mi[eé]rcoles/i) || textoNormalizado === "miércoles" || textoNormalizado === "miercoles") fechaBase = nextWednesday(ahora);
//     else if (textoNormalizado.match(/pr[oó]ximo\s+jueves/i) || textoNormalizado === "jueves") fechaBase = nextThursday(ahora);
//     else if (textoNormalizado.match(/pr[oó]ximo\s+viernes/i) || textoNormalizado === "viernes") fechaBase = nextFriday(ahora);
//     else if (textoNormalizado.match(/pr[oó]ximo\s+s[aá]bado/i) || textoNormalizado === "sábado" || textoNormalizado === "sabado") fechaBase = nextSaturday(ahora);
//     else if (textoNormalizado.match(/pr[oó]ximo\s+domingo/i) || textoNormalizado === "domingo") fechaBase = nextSunday(ahora);
//     else if (textoNormalizado) {
//         const formatosComunes = ['d \'de\' MMMM \'de\' yyyy', 'd \'de\' MMMM', 'dd/MM/yyyy', 'd/M/yy', 'yyyy-MM-dd', 'P', 'PP'];
//         for (const fmt of formatosComunes) {
//             try {
//                 const d = dateFnsParse(textoNormalizado, fmt, ahora, { locale: es });
//                 if (isValid(d) && d >= hoy) {
...
//         }
//     }
//     console.log(`[parsearFechaHoraInteligente] Fecha base determinada: ${fechaBase ? formatISO(fechaBase) : 'null'}`);

//     if (fechaHoraStrEntrada && !fechaBase && !horaExtraida) {
//         try {
//             const d = new Date(fechaHoraStrEntrada);
//             if (isValid(d) && d > ahora) {
//                 console.log(`[parsearFechaHoraInteligente] Parseado con new Date(): ${formatISO(d)}`);
//                 return d; // Si es una fecha completa válida, la devolvemos
//             } else if (isValid(d) && opciones?.permitirSoloFecha && d >= hoy) {
//                 console.log(`[parsearFechaHoraInteligente] Parseado con new Date() (solo fecha permitida): ${formatISO(startOfDay(d))}`);
//                 return startOfDay(d);
//             }
//         } catch { /* ignorar */ }
//     }

//     // MODIFICADO: Lógica para manejar permitirSoloFecha
//     if (fechaBase && !horaExtraida) {
//         if (opciones?.permitirSoloFecha) {
//             console.log(`[parsearFechaHoraInteligente] Solo fecha permitida y encontrada: ${formatISO(fechaBase)}`);
//             return fechaBase; // Devuelve startOfDay(fechaBase)
//         } else {
//             console.warn(`[parsearFechaHoraInteligente] Fecha proporcionada ('${fechaHoraStrEntrada}') sin hora específica, y se requiere hora.`);
//             return null;
//         }
//     }
//     if (!fechaBase && horaExtraida) {
//         console.warn(`[parsearFechaHoraInteligente] Hora proporcionada ('${fechaHoraStrEntrada}') sin fecha clara, se requiere fecha.`);
//         return null;
//     }
...
//             console.log(`[parsearFechaHoraInteligente] Parseado final: "${fechaHoraStrEntrada}" -> ${formatISO(fechaHoraFinal)}`);
//             return fechaHoraFinal;
//         } else {
//             console.warn(`[parsearFechaHoraInteligente] Fecha/hora parseada es inválida o está en el pasado estricto: ${formatISO(fechaHoraFinal)}`);
//             return null;
//         }
//     }

//     console.warn(`[parsearFechaHoraInteligente] No se pudo parsear: "${fechaHoraStrEntrada}"`);
//     return null;
// }

// Dentro de app/admin/_lib/funciones/agendarCita.actions.ts
