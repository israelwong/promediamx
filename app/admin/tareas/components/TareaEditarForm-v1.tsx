...
export default function TareaEditarForm({ tareaId }: Props) {
    ...
    // --- Cargar parámetros de función ---
    const fetchParametrosParaFuncion = useCallback(async (funcionId: string | null | undefined) => {
        // ... (sin cambios) ...
        if (!funcionId) { setParametrosDeFuncion([]); return; }
        setLoadingParams(true);
        setError(null);
        try {
            const params = await obtenerParametrosPorFuncionId(funcionId);
            setParametrosDeFuncion(params || []);
        } catch (err) {
            console.error("Error fetching function parameters:", err);
            setError("Error al cargar parámetros de la función seleccionada.");
            setParametrosDeFuncion([]);
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            ...
            try {
                const [tareaData, categoriasData, canalesData, etiquetasData, funcionesData] = await Promise.all([
        ...
        fetchInitialData();
    }, [tareaId, fetchParametrosParaFuncion]);
        ...
        try {
            ...
            setTareaOriginal((prev: TareaOriginalConDetalles | null) => {
                ...
                const newState: TareaOriginalConDetalles = {
                    ...
                    tareaFuncion: dataToSend.tareaFuncionId
                        ? (funciones.find(f => f.id === dataToSend.tareaFuncionId) as TareaFuncion & Pick<TareaFuncion, "id" | "nombreVisible">) ?? null
                        : null,
                    // Mantener campos que no se envían como _count
                    _count: prev._count
                };
                return newState;
            });

            // Opcional: Refrescar formData si la acción devuelve la tarea completa actualizada
            // setFormData(prev => ({ ...prev, ...tareaActualizada }));
        ...
        <div>
            ...
            <form onSubmit={handleSubmit} id="edit-tarea-form" className="grid grid-cols-1 lg:grid-cols-3 gap-6" noValidate>
                ...
                <div className="lg:col-span-1 flex flex-col space-y-5">
                    {/* Icono Tarea */}
                    <div>
                        {/* Renderiza el componente TareaIcono aquí */}
                        <TareaIcono tareaId={tareaId} iconoUrl={formData.iconoUrl} />
                    </div>

                    {/* Resto de la columna 1 (sin cambios)... */}
                    ...
                    <div className={fieldGroupClasses}>
                        <h3 className={sectionTitleClasses}><Settings2 size={16} className="text-zinc-400" /> Ejecución de la función API</h3>
                        <div>
                            <label htmlFor="tareaFuncionId" className={labelBaseClasses}>Función Automatización (Opcional)</label>
                            <select id="tareaFuncionId" name="tareaFuncionId" value={formData.tareaFuncionId || ''} onChange={handleChange} className={selectBaseClasses} disabled={isSubmitting}>
                                <option value="">-- Ninguna --</option>
                                {funciones.map(func => (<option key={func.id} value={func.id}>{func.nombreVisible} ({func.nombreInterno})</option>))}
                            </select>
                        </div>
                        {(formData.tareaFuncionId || loadingParams) && (
                            <div className="mt-3 space-y-2">
                                <h4 className="text-xs font-medium text-zinc-300">Parámetros Estándar Requeridos:</h4>
                                {loadingParams ? (
                                    <p className="text-xs text-zinc-400 italic flex items-center gap-1"><Loader2 size={12} className='animate-spin' /> Cargando...</p>
                                ) : parametrosDeFuncion.length === 0 ? (
                                    <p className="text-xs text-zinc-400 italic">Sin parámetros estándar.</p>
                                ) : (
                                    <div className="flex flex-wrap gap-1.5">
                                        {parametrosDeFuncion.map(param => (
                                            <span key={param.id} className={paramTagClasses} title={`ID: ${param.nombreInterno} | Tipo: ${param.tipoDato}`}>
                                                {param.nombreVisible}
                                            </span>
                                        ))}
                                    </div>
