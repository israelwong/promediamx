...
import { Loader2, ListChecks, PlusIcon, PencilIcon, Trash2, Save, XIcon, GripVertical, ListTree } from 'lucide-react'; // Iconos
...
function SortableCategoryItem({ categoria, onEditClick }: { categoria: CategoriaConOrden, onEditClick: () => void }) {
    ...
    return (
        <li ref={setNodeRef} style={style} className={listItemClasses}>
            <button {...attributes} {...listeners} data-dndkit-drag-handle className="cursor-grab touch-none text-zinc-500 hover:text-zinc-300 flex-shrink-0 focus:outline-none focus:ring-1 focus:ring-blue-500 rounded" aria-label="Mover categoría"><GripVertical size={18} /></button>
            <div className="flex-grow mr-2 overflow-hidden">
                <p className="text-sm font-medium text-zinc-200 truncate" title={categoria.nombre}>{categoria.nombre}</p>
                {categoria.descripcion && <p className="text-xs text-zinc-400 line-clamp-1" title={categoria.descripcion}>{categoria.descripcion}</p>}
            </div>
            <button onClick={onEditClick} className={buttonEditClasses} title="Editar Categoría"><PencilIcon size={16} /></button>
        </li>
    );
}
...
export default function TareasCategorias() {
    ...
    const fetchCategorias = useCallback(async (isInitialLoad = false) => {
        ...
        try {
            ...
            setCategorias((data || []).map((cat, index) => ({ ...cat, orden: cat.orden ?? index + 1 })));
        } catch (err) {
    ...
    const handleModalDelete = async () => {
        ...
        if (confirm(`¿Estás seguro de eliminar la categoría "${categoriaParaEditar.nombre}"?`)) {
            setIsSubmittingModal(true); setModalError(null);
            try {
    ...
    const handleDragEnd = useCallback(async (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            const finalOrder = reordered.map((cat, index) => ({ ...cat, orden: index + 1 }));
    ...
    return (
        <div className={containerClasses}>
            ...
            <div className={listContainerClasses}>
                {loading ? (
                    <div className="flex items-center justify-center py-10 text-zinc-400"><Loader2 className="h-5 w-5 animate-spin mr-2" /><span>Cargando categorías...</span></div>
                ) : categorias.length === 0 && !error ? (
                    <div className="flex flex-col items-center justify-center text-center py-10"><ListChecks className="h-8 w-8 text-zinc-500 mb-2" /><p className='text-zinc-400 italic text-sm'>No hay categorías definidas.</p></div>
                ) : (
                ...
                <div className={modalOverlayClasses} onClick={closeModal}>
                    <div className={modalContentClasses} onClick={(e) => e.stopPropagation()}>
                        <div className={modalHeaderClasses}>
                            <h3 className="text-lg font-semibold text-white">{modalMode === 'create' ? 'Crear Nueva Categoría' : 'Editar Categoría'}</h3>
                        ...
                        <form onSubmit={handleModalFormSubmit}>
                            <div className={modalBodyClasses}>
                                ...
                                <div>
                                    <label htmlFor="modal-nombre" className={labelBaseClasses}>Nombre Categoría <span className="text-red-500">*</span></label>
